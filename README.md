...
- **💾 水印模板系统 (GUI)**：
    - **模板保存**：将当前的所有水印设置（水印类型、内容、字体、颜色、位置、大小、透明度、旋转角度等）一键保存为模板。
    - **模板管理**：通过可视化界面查看、加载和删除已保存的模板，模板按保存时间排序显示。
    - **快速切换**：双击模板名称即可快速应用模板设置。
    - **设置自动恢复**：程序启动时自动加载上一次关闭时的所有设置，无需重新配置。
    - **模板备份与分享**：模板以 JSON 格式存储，便于备份和在不同设备间分享配置。
...
6.  **水印模板操作**：
    - **保存模板**: 
        - 当您满意当前的水印设置时，点击 **"保存模板"** 按钮。
        - 所有当前设置（水印类型、内容、样式、位置、角度等）将被保存为一个模板文件。
        - 模板文件以时间戳命名，存储在 `templates/` 目录下。
    - **加载模板**:
        - 点击 **"加载模板"** 按钮，选择之前保存的模板文件。
        - 或者点击 **"管理模板"** 按钮打开模板管理界面。
    - **模板管理界面**:
        - 显示所有已保存的模板，按保存时间排序。
        - **双击** 模板名称可快速加载该模板的所有设置。
        - 选中模板后点击 **"重命名模板"** 可为模板设定自定义名称，便于识别和管理。
        - 选中模板后点击 **"删除模板"** 可删除不需要的模板。
        - 加载模板后，界面会自动切换到对应的水印类型，并更新所有相关设置。
    - **自动恢复**: 程序启动时会自动加载上次关闭时的设置，无需重新配置。
